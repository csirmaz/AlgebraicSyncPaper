...
% TODO include command in def
\begin{mydef}[Replacement operator]
Using $\FS[\valv/n]$ we denote a filesystem derived from $\FS$:
\[ \FS[\valv/n](m) :=
   \begin{cases}
   \valv &\mbox{if~} m=n\\
   \FS(m) &\mbox{otherwise.}
...
The effect of the commands is as follows:
\begin{align*}
&\cbrk\aFS = \fsbroken \\
&\cxynv\aFS = 
   \begin{cases}
