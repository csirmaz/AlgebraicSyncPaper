...
may be $create(n,\valv)$---create the value $\valv$ at node $n$,
$edit(n,\valv)$---modify the value at $n$ to become $\valv$,
and $remove(n)$---delete the value at node $n$; 
...
in a the following format:
\[ \cxynv, \]
where $X$ is the input type, $Y$ is the output type, $n$ is the node,
and $\valvy$ is a value of the appropriate type from $\setvx{Y}$.
We use $\cchard$, $\ccharf$ or $\ccharb$ to represent the different
types. For example, $\cbdaa{n}{\vald}$ 
...
or simply $S\FS$ if $S$ is a sequence of commands. % TODO Add space between command and FS
