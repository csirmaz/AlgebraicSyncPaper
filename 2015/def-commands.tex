...
edits that result in directories ($edit(n,D)$) and
...
before the command is applied. In particular, $create(n,V)$ requires that there are no files
...
($\cxynv=\czwmv$)
implies
that their nodes and output value is also the same.
