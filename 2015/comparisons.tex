...
As a test of the proposed reconciliation algorithm, we compare our definition of conflicting
updates to how conflicts are defined by Balasubramaniam and Pierce (\cite{BP}).
The update detector described in \cite{BP} provides a safe estimate of nodes
(paths) at which updates occurred.
It marks some nodes as \emph{dirty} in a way that we know that at non-dirty paths
the filesystem has not changed between its original and current states
(see their Definition 3.1.1).
The \emph{dirty} marks are also up-closed, that is, all ancestor nodes of a dirty node
are also dirty (their Fact 3.1.3).
...
detects a conflict, it entails a conflict based on the \emph{dirty}
property.
Balasubramaniam and Pierce also suppose that all directories are equal,
therefore we are safe to continue to assume that $|\setd|=1$.
...
as the filesystem changes at $n$ in $\FS_A$ and at a descendant in $\FS_B$
(none of the commands are assertion commands).

We now only need to show that $\FS_A(n)\neq\FS_B(n)$, as from this we will also
know that at least one of these values is not a directory.
If $n=m$, this follows from the fact that $\cxynv\neq\czwmv$ (as they are not in
$A\cap B$), which in this case can only be if $\FS_A(n)=\valvy\neq\valvw=\FS_B(n)$.
...
$\FS_A(n)\neq\FS(n)=\FS_B(n)$.
The final case is when there is a command on $n$ in $B$; let it be $\cqrnv$.
