...
The partial ordering determined by the parent relation is denoted by $\descendant$:
we write $n\descendant m$ if $n$ is the ancestor of $m$ ($n=\parent^n(m)$),
that is, the path name of $n$ is an initial segment of that of $m$. 
We write $n\descendantEq m$ if $n\descendant m$ or $n=m$.
\end{mydef}

\begin{mydef}[$n\unrel m$]
We write $n\unrel m$, or ``$n$ and $m$ are incomparable'' if $n\not\descendantEq m$ and $n\not\ancestorEq m$.
incomparable nodes are on different branches or on different trees;
...
% TODO We will only use these except for...
...
\begin{mydef}[$A\indep B$: Independent commands, sequences and sets]
...
and sequences as for incomparable nodes. As we will in the next
