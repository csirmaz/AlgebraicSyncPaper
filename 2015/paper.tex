...
\newcommand{\worksc}[2]{\workssign({#1}|{#2})}
...
This is equivalent to the proposition that for simple sequences $\extset{S} = \{S'|S\equiv S'\}$.
\end{mylem}
\begin{proof}
TODO
% TODO
...
% TODO orderset only defined on simple
...
if $A_1\in\seqset{A}$, then
$ \worksc{\seqset{A}}{\seqset{B}} \wedge \worksc{\seqset{C}}{A_1} \Rightarrow \worksc{\seqset{A}\cup\seqset{C}}{\seqset{B}}$.
\end{mycor}
...
By definition, if the two commands are not independent, then one of the following holds:
\begin{itemize}
...
Second, if there the input type does not match the filesystem,
...
that contains the commands from $S$ and $T$ preserving their original order,
but in any permutation between $S$ and $T$ is equivalent to $S\cc T$ on all filesystems.

Let the command that breaks $\FS$ in $T$ be $t$
...
From this and \cref{workschained} it follows that:

\begin{mycor}\label{r_indep_combine}
% An inference rule in the algebra
The combination of independent sequences continues to work under the same conditions:
\[ \worksc{S}{\seqset{U}} \wedge \worksc{T}{\seqset{U}} \wedge S\indep T \Rightarrow \worksc{S\cc T}{\seqset{U}} \]
\end{mycor}
...
in reverse order.
%
Clearly, $(S^{-1})^{-1}\equiv S$ for any sequence.
\end{mydef}
...
The combination of sequences with a common head and independent tail continues to work:
...
and from \cref{r_indep_combine} we know
$\worksc{B\cc C}{A^{-1}\cc \seqset{S}}$. Applying the second line of \cref{r_invmove} we get
$\worksc{(A^{-1})^{-1}\cc B\cc C}{\seqset{S}}$, which proves our lemma as $(A^{-1})^{-1}\equiv A$.
\end{proof}
...
and from \cref{simple_is_max,lemma:minextorder} we know that $B'\equiv B$.
