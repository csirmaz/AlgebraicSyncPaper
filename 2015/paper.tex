...
This is equivalent to the proposition that $\extset{S} = \{S'|S\equiv S'\}$.
\end{mylem}
\begin{proof}
TODO
% TODO
...
and $\recb$ can be obtained in a similar way by reversing $A$ and $B$.
\end{mydef}

We would now like to prove that $\reca$ and $\recb$ can be applied to the replicas,
that is, without loss of generality,
\[ \reca\FS_B\neq\fsbroken. \]
So that we can formalise statements needed to prove this,
...
% An inference rule in the algebra
$\forall A,S: \worksc{A}{A\cc S}$
\end{mycor}

\begin{mycor}
% An inference rule in the algebra
$\worksc{S}{T}$ can be chained:
...
\begin{mylem}\label{r_indep_combine}
% An inference rule in the algebra
\[ \worksc{T}{S} \wedge \worksc{U}{S} \wedge T\indep U \Rightarrow \worksc{T\cc U}{S} \]
\end{mylem}
\begin{proof}
% TODO Proof
TODO
...
\begin{gather*}
\worksc{B\cc A}{S} \Rightarrow \worksc{A}{B^{-1}\cc S} \\
\worksc{A}{B\cc S} \Rightarrow \worksc{B^{-1}\cc A}{S}
\end{gather*}
% TODO Proof
TODO.
...
\[ \worksc{A\cc B}{S} \wedge \worksc{A\cc C}{S} \wedge B\indep C \Rightarrow \worksc{A\cc B\cc C}{S} \]
\end{mylem}
\begin{proof}
Based on \cref{r_invmove} we know
$\worksc{B}{A^{-1}\cc S}$ and $\worksc{C}{A^{-1}\cc S}$,
and from \cref{r_indep_combine} we know
$\worksc{B\cc C}{A^{-1}\cc S}$. Applying the second line of \cref{r_invmove} we get
$\worksc{(A^{-1})^{-1}\cc B\cc C}{S}$, which proves our lemma as $(A^{-1})^{-1}\equiv A$.
\end{proof}
...
\end{myth}
where, to restate the definition,
{ % scope
...
We show that these can be reversed resulting in an equivalent sequence.
...
In these cases, either $m=\parent(n)$ or $p=\parent(m)$.
...
$\worksc{(A\cap B)\cc (B\setminus A)\cc \reca}{A,B}$.
As we already know that $\reca\indep (B\setminus A)$
and that $\worksc{(A\cap B)\cc(B\setminus A)}{A,B}$
(as $\worksc{B}{A,B}$),
based on \cref{indep_prefix_combine}
we only need to prove that
$\worksc{(A\cap B)\cc \reca}{A,B}$.

% ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

\newcommand{\acbi}{(A\cap B)^{-1}}
...
$\worksc{S^*}{C_1,\ldots,C_n}$ as $\worksc{S'}{C_1,\ldots,C_n}$ and .
