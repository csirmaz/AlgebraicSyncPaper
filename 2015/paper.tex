...
We will also prove the following lemmas.
...
% An inference rule in the algebra
\[ \worksc{T}{\seqset{S}} \wedge \worksc{U}{\seqset{S}} \wedge T\indep U \Rightarrow \worksc{T\cc U}{\seqset{S}} \]
\end{mylem}
\begin{proof}
% TODO Proof
TODO
...
$\cxy(n,V_Y)\not\indep\czw(m,V_W) \wedge n\unrel m$ would lead to contradiction.
...
Next, we prove that
$\cxy(n,V_Y)\indep\czw(m,V_W) \wedge n\not\unrel m$ would lead to contradiction.
As $n\not\unrel m$
...
as $\worksc{A,B}{A,B}$
and $A\equiv\acb\cc\amb$, $B\equiv\acb\cc\bma$.
\end{itemize}
