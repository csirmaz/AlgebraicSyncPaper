...
means that $\wrksx{\cxynv\cc\czwmv}$ is a construction pair, which is a contradiction.
...
means that $\wrksx{\cxynv\cc\czwmv}$ is a destruction pair, which is a contradiction.
\end{proof}
...
\begin{mydev}[$\orderd{S}$]
It follows from \cref{simple_reorder_equiv} that given a simple set of commands $S$,
which can be applied to a filesystem without breaking it,
all sequences in $\orderset{S}$
...
We write $\orderd{S}$ to represent this endofunction.
\end{mydev}
