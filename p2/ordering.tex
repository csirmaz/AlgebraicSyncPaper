...
TODO MOVE TO AMONG THE AXIOMS TO AVOID DUPLICATION.
BUT THIS NEEDS THE DEFINITION OF SIMPLE SEQUENCES.
...
We use an inverse proof and assume $\wrks{S'}$.
...
\[ \orderset{S} = \{T \whr T\equiv S \wedge T \mbox{~is simple} \}. \]
\end{mylem}
...
% TODO APPENDIX
\begin{proof}
First, we prove, by contradiction, that if a simple sequence $T$, $T\equiv S$, then $T\in\orderset{S}$.
...
$[y_\indx]=[\vald]$ must be true by the definition of $\ordersetsign$.
\end{proof}
