...
We can derive this from our filesystem model in the following way:
we know if $y\neq\vald$ then the commands will break every filesystem $\FS$
as either before or after $\czwnv$, $\FS(n)$ is not empty, and therefore
at its parent $\nn$ there must be a directory.
We also know if $[z]\neq[\empt]$ then the commands will again break every filesystem
as either before or after $\cxynnv$, $\FS(\nn)$ is not a directory.
\end{myax}
...
The derivation of this \namecref{ax_directparent_breaks} is similar to that of \cref{ax_directchild_breaks}.
We know if $y\neq\empt$ then the commands will break every filesystem $\FS$
as either before or after $\czwnnv$, $\FS(\nn)$ is not a directory, and therefore
every child of $\nn$ must be empty.
We also know if $[z]\neq[\vald]$ then the commands will again break every filesystem
as either before or after $\cxynv$, $\FS(n)$ is not empty.
\end{myax}
