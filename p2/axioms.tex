...
We also see that $\cxynv\cc\czwnv\equiv\czwnv\cc\cxynv$ can only happen if $w=y$.
Therefore $n=m$, $[x]=[y]=[z]=[w]$ and $w=y$, which contradicts our assumption that the commands are different.

TODO say not assertion and not replacement?
...
$\cxynv\cc\czwmv$ breaks all filesystems as they are not assertion commands,
