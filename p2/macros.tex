...
\newcommand{\nn}{n'} % parent node
\newcommand{\T}{{}^t} % type-equivalent class mapping
...
\newcommand{\cTaaa}[3]{\langle\langle{#1,#2,#3}\rangle\rangle}
...
\newcommand{\worksT}[1]{\workssign\T({#1})} % works
\newcommand{\workscT}[2]{\workssign\T({#1}\mathrel{|}{#2})} % works on condition
