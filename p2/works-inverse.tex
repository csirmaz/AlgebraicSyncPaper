...
As a special case, we write $\worksmnil{\sqs{A}}$ if we know that there is at least one filesystem
...
only requires that $B$ is defined where $A$ is defined, where the latter also requires
...
By definiton, for all objects where it is defined, $a\T\typeeq a$.
% $|\setf|$ is also $1$.
...
It is easy to see that $\worksm{\ia}{B\cc\ic} \Leftrightarrow \worksm{\ia}{\{B\}}$.
