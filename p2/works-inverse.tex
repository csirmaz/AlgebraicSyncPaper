...
\begin{itemize}
\item For two filesystems we write $\FS\typeeq\GS$ iff they are both broken, or they share the same nodes $\setn$,
and for all $n\in\setn$, $[\FS(n)]=[\GS(n)]$.
\item For two commands we write $\cxynv\typeeq\czwnv$ iff 
$[x]=[z]$ and $[y]=[w]$ and $n=m$.
...
but as $|\setb|=|\setd|=1$, this is only possible at values of the type $[\valf]$.
We select an arbitrary file value $\fil\in[\valf]$, and choose a representative
of each $\typeeq$-equivalence class where all (if any) values of the type $[\valf]$ is $\fil$.
For example, in the case of commands, the representative of $[\cbfaa{n}]_\typeeq$ is $\caaa{\empt}{\fil}{n}$.
...
For example, $\cbfaa{n}\T = \caaa{\empt}{\fil}{n}$,
