...
(also known as semantic scheduling)
as opposed to syntactic scheduling that follows the strict temporal order
of updates and may give rise to otherwise resolvable conflicts (\cite{SSH}).
...
it plays a central role (\cite{SSH});
in IceCube multiple orders are tested to find an acceptable one (\cite{KRSD}, see also \cite{MPV}),
